...
export function autoTune(explicit: Partial<TunedValues>): TunedValues {
  ...
  if (cpus >= 8 && ramGb > 8) {
    defaults = { validationConcurrency: 1000, maxConfigsPerCycle: 1000, tcpTimeoutMs: 2500 };
  } else if (cpus >= 4 && ramGb > 4) {
    defaults = { validationConcurrency: 600, maxConfigsPerCycle: 1000, tcpTimeoutMs: 2500 };
  } else if (cpus >= 2 && ramGb > 2) {
    defaults = { validationConcurrency: 300, maxConfigsPerCycle: 1000, tcpTimeoutMs: 2500 };
  } else {
    defaults = { validationConcurrency: 100, maxConfigsPerCycle: 1000, tcpTimeoutMs: 3000 };
  }
